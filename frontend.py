...
def bottom_frame_insert(columns: [], current_row: str = None, disable_flag: int = 0) -> {}:
	"""
...
def addRowToTable(entries, windowToDestroy):
	...
	if not selected_cell:
		...
		for i, c in enumerate(columns):
			if controller.checkIfStr(selected_table, c):
				values[i] = f'"{values[i]}"'
		controller.insertRowToTable(selected_table, columns, values)
