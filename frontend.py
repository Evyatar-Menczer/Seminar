...
def drop_DB():
    ...
    controller.drop_DB()
    present_new_trees()
    msg_label.config(text=success_msgs["clear_db"], anchor=CENTER)
...
def drop_table_popup() -> None:
    ...
    message = messagebox.askquestion(f'Drop Table {selected_table} ', warn_msgs["drop_tbl_warning"],icon='warning')
    if message == 'yes':
        drop_table()
...
def drop_db_popup() -> None:
    """
        Pop up window to confirm the users action
    """
    message = messagebox.askquestion('WARNING! - Drop Data Base', warn_msgs["drop_db_warning"], icon='warning')
    if message == 'yes':
        drop_DB()
...
def delete_row_popup() -> None:
    ...
    if not selected_cell:
        ...
        return
    message = messagebox.askquestion('Delete Row', warn_msgs["row_delete_warning"],icon='warning')
    if message == 'yes':
        delete_entire_row()
...
def insert_new_row_to_table(entries: dict, window_to_close) -> None:
    ...
    present_new_trees()
    msg_label.config(text=success_msgs["row_insert"], fg='#20B519', anchor=CENTER)
